...
public:
    ...
    : Node("nav_server")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Nav Server initialized (Nav2 forwarding)");
    }
...
private:
    ...
    // Optional: set_initial_pose is not used here, but kept for possible expansion.
    void set_initial_pose(double x, double y) {
        ...
        RCLCPP_INFO(this->get_logger(), "Set initial pose at (%.2f, %.2f)", x, y);
    }
};
